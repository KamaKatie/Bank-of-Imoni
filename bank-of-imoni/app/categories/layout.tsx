...
export default function RootLayout({ children }: RootLayoutProps) {
  ...
  return (
    <div className="h-full flex w-full">
      <aside className="hidden md:flex md:w-64 flex-col border-r h-full">
            ...
            {categories?.map((category) => {
              ...
              return (
                <Link key={category.id} href={href}>
                  <div
                    className={cn(
                      "flex gap-3 items-center p-2 border-b mx-4 my-1 rounded-xl transition-all h-full",
                      "hover:bg-accent",
                      isActive
                        ? "bg-muted font-medium border-b shadow-sm"
